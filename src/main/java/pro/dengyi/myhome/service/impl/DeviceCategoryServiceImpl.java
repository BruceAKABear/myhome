...
public class DeviceCategoryServiceImpl implements DeviceCategoryService {
    ...
    public void addUpdate(DeviceCategory deviceCategory) {
        if (ObjectUtils.isEmpty(deviceCategory.getId())) {
            ...
            categoryFieldDao.delete(new LambdaQueryWrapper<CategoryField>().eq(CategoryField::getCategoryId, deviceCategory.getId()));
        }
    ...
    public void delete(String id) {
        List<Device> devices = deviceDao.selectList(new LambdaQueryWrapper<Device>().eq(Device::getCategoryId, id));
    ...
    public IPage<DeviceCategory> page(Integer pageNumber, Integer pageSize, String name) {
        IPage<DeviceCategory> page = new Page<>(pageNumber == null ? 1 : pageNumber, pageSize == null ? 10 : pageSize);
        ...
        if (!CollectionUtils.isEmpty(records)) {
            for (DeviceCategory record : records) {
                record.setDeviceCount(Math.toIntExact(deviceDao.selectCount(new LambdaQueryWrapper<Device>().eq(Device::getCategoryId, record.getId()))));
                record.setCategoryFieldList(categoryFieldDao.selectList(new LambdaQueryWrapper<CategoryField>().eq(CategoryField::getCategoryId, record.getId())));
            }

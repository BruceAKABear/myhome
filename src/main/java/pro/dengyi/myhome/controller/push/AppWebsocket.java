...
import pro.dengyi.myhome.common.utils.RequestSourceHolder;
...
public class AppWebsocket {
    ...
    public static final AtomicInteger ONLINE_DEVICES_COUNT = new AtomicInteger(
            0);
     ...
     * @return
     */
    public static Boolean sendMessage2Device(@NotBlank String userId,
                                             @NotBlank String message) {
        log.info("服务端给用户:" + userId + "发送消息，消息内容为:" + message);
        boolean flag = true;
        if (StringUtils.isNotBlank(userId) && devicesInfoMaps.containsKey(
                userId)) {
            try {
                devicesInfoMaps.get(userId).session.getBasicRemote()
                        .sendText(message);
            } catch (IOException e) {
                log.error(
            ...
            log.error(
                    "服务端给用户:" + userId + "发送消息，消息内容为:" + message + ",发送时错误，错误信息为:设备不在线");
            flag = false;
        }
        return flag;
    }
    ...
    public static Boolean sendMessage2Device(@NotBlank String message) {
        ...
        devicesInfoMaps.keySet().forEach(key -> {
            try {
                devicesInfoMaps.get(key).session.getBasicRemote()
                        .sendText(message);
            } catch (IOException e) {
                log.error("服务端给用户:" + devicesInfoMaps.get(
                                key).user.getId() + "发送消息，消息内容为:" + message + ",发送时错误，错误信息为:",
                        e);
            }
        });
        return flag;
    ...
    public void onOpen(Session session, @PathParam("token") String token) {
        if (!ObjectUtils.isEmpty(token)) {
            ...
            RequestSourceHolder.setSource(RequestSourceEnum.PHONE);
    ...
    public void onClose() {
        ...
        log.info("移动端断开websocket,当前连接用户为:{},断开后在线用户数为:{}",
                this.user.getId(), AppWebsocket.ONLINE_DEVICES_COUNT.get());
    }
    ...
    public void onMessage(String message, Session session) {
        log.info("接收到移动端发送的消息,发送人为:{},消息内容为:{}",

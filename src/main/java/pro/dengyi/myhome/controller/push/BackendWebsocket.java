...
@Component
@Slf4j
@ServerEndpoint(value = "/socket/backend/{token}")
public class BackendWebsocket {

    private static final AtomicInteger onlineDevicesCount = new AtomicInteger(0);
    private static final ConcurrentHashMap<String, BackendWebsocket> devicesInfoMaps = new ConcurrentHashMap<>();
    ...
    public void onOpen(Session session, @PathParam("token") String token) {
        if (!ObjectUtils.isEmpty(token)) {
            ...
            if (devicesInfoMaps.containsKey(user.getId())) {
                ...
                devicesInfoMaps.put(user.getId(), this);
                BackendWebsocket.onlineDevicesCount.getAndIncrement();
            }
            log.info("移动端连接websocket,当前连接用户为:{},连接后在线用户数为:{}", this.user.getId(),
                    BackendWebsocket.onlineDevicesCount.get());
        }
    }
    ...
    public void onClose() {
        if (devicesInfoMaps.containsKey(this.user.getId())) {
            ...
            BackendWebsocket.onlineDevicesCount.getAndDecrement();
        }
